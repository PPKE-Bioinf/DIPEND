...
struct Line{
    ...
    ofstream lysls ("./conly-left/LYSl.bin", ios::out |   ios::binary);
    if(!lysls) {
        cout << "Cannot open file!" << endl;
        return 1;
   }

    ofstream metls ("./conly-left/METl.bin", ios::out |   ios::binary);
    if(!metls) {
        cout << "Cannot open file!" << endl;
        return 1;
   }

    ofstream phels ("./conly-left/PHEl.bin", ios::out |   ios::binary);
    if(!phels) {
        cout << "Cannot open file!" << endl;
        return 1;
   }

    ofstream prols ("./conly-left/PROl.bin", ios::out |   ios::binary);
    if(!prols) {
        cout << "Cannot open file!" << endl;
        return 1;
   }
    ...
    while (std::getline(infile_conly, line_conly))
    {
        if (line_conly.size()>63 && line_conly.substr(0,1)!="#"){
            ...
            if (l.neighbour[0]=='l' && l.neighbour[1]=='e' && l.neighbour[2]=='f' && l.neighbour[3]=='t'){
                ...
                if (l.firstaa[0]=='L' && l.firstaa[1]=='Y' && l.firstaa[2]=='S'){lysls.write((char *) &l, sizeof(Line));}
                if (l.firstaa[0]=='M' && l.firstaa[1]=='E' && l.firstaa[2]=='T'){metls.write((char *) &l, sizeof(Line));}
                if (l.firstaa[0]=='P' && l.firstaa[1]=='H' && l.firstaa[2]=='E'){phels.write((char *) &l, sizeof(Line));}
                if (l.firstaa[0]=='P' && l.firstaa[1]=='R' && l.firstaa[2]=='O'){prols.write((char *) &l, sizeof(Line));}
            } // end if left
            } // end if line length
        } // end while getline
